...
export default function Leaderboard() {
  ...
  return (
    <ClickableCard title={"Leaderboard"} handleClick={handleClick}>
      <Carousel
        axis="vertical"
        autoPlay
        interval={2000}
        ...
        showThumbs={false}
      >
        {Array.from({ length: leaders.length / 3 }, (_, index) => index).map((leader, sliceIdx) => (
          <div key={sliceIdx} className="h-[120px] px-2">
            <table className="table no-border">
              <tbody>
                {leaders.slice(sliceIdx * 3, sliceIdx * 3 + 3).map((leader, i) => (
                  <Leader
                    key={i}
                    rank={leader.rank}
